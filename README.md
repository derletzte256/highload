...
| Ко-фоллоу (оффлайн) | на основе векторов, где мы можем посчитать сходимость меры Жаккара для уже подписанных с остальными каналами. Исходя из этого для каждого канала в sim:channel:{channel_id} сохраняем похожие | 100 (на каждый канал) |
| Ко-фоллоу (онлайн) | Получаем из всех подписанных похожие на основе кэша ко-фоллоу | до 10000 (10 * 1000) |
| Ранжирование | рассчет по формуле score для всех кандидатов и сортировка | 10100 -> 200 TTL 5 минут |

score = 2.5*(подписан) + 0.35*log1p(зрителей сейчас) + 1.0*sim_cfollow + 0.10*(буст, если канал - партнер твича) + 0.20*(был повтор, уже показывали данные канал менее 24 часов назад) + 0.05*(совпадение языка аккаунта и стрима)
