...
| ------------------ | -------- | -------------------- |
| Резервирование ресурсов (CPU, RAM) | Дополнительные вычислительные мощности, который смогут принять нагрузку в пиковых сценариях | 1.2х от пиковых |
| Резервирование физических компонентов (сервера, диски и т.п.) | Дополнительное оборудование для замены  | ЗИП в зависимости от отказа серверов |
...
## 10. Схема проекта
<img src="highload.drawio.svg" />

## 11. Базовый расчёт аппаратных ресурсов

Расчет строится на разделении сервиса на Control Plane (API, Чат, Отображение - упор на RAM/RPS) и Data Plane (Видео - упор на CPU/Network).

### Удельное потребление ресурсов
| Сервис | Метрика | Потребление на 1 ядро | Комментарий |
| --- | --- | --- | --- |
| API | 1 RPS | 1 500  | Легкая бизнес-логика |
| Chat | 1 соединение | 25 000 | WebSocket |
| Edge Node | 1 Гбит/c | 4 | Лимит не в CPU, а в пропускной способности |
| Transcode | 1 Stream | 0.5 стрима | Транскодинг 1080p в 4 качества (CPU) |

### Расчет нагрузки
Пики нагрузки:
- 89K стримов (все транскодируются) 
- 8.4 Тбит/с трафика раздачи.

1.  Видео-процессинг (Ingest + Transcoding):
    - Транскодинг 89 000 стримов одновременно в 4 качества.
    - Нагрузка: 89 000 стримов * 2 ядра/стрим = 178 000 ядер.
2.  Раздача видео (Edge):
    - Нагрузка: 8 400 Гбит/с.
    - Эффективная емкость 1 сервера (порт 2x25GbE) = 40 Гбит/с.
    - Серверов: 8400 / 40 = 210.
3.  API и Чат:
    - Нагрузка API: 5000 RPS (с запасом) - не критично.
    - Чат: 3M соединений -> 120 ядер и 512 ГБ RAM суммарно.

## Модель развертывания и хостинга
- Kubernetes (Control Plane): API, Чат, Web. Обеспечивает удобство деплоя и масштабирования.
- Bare Metal (Data Plane): Видео-серверы и БД.

Хостинг: Аренда выделенных серверов (Dedicated) в географически распределенных ДЦ (для минимизации задержек и стоимости трафика).

## Конфигурации серверов
Для унификации выбираем 3 типа серверов на базе AMD EPYC. Закладываем резерв 20%.

### Тип 1 (Ingest + Транскодинг)
- Задача: Ingest потоков и тяжелый транскодинг x264.
- Spec: 2x EPYC 7763 (128 ядер / 256 потоков), 256 GB RAM.
- Мощность: 60 стримов полного цикла на сервер.
- Расчет: 89 000 / 60 * 1.2 = 1 780 серверов.
- Стоимость: ~1250$ в месяц

### Тип 2: (Edge)
- Задача: Отдача RTMP
- Spec: 1x AMD EPYC 7402P (24 ядра), 128 GB RAM, 2x25 Gbps Uplink.
- Мощность: 100 Гбит/с эффективного трафика.
- Расчет: 8 400 / 40 * 1.2 = 252 сервера.
- Стоимость: ~1250$ в месяц

### Тип 3: (K8s Nodes & DB)
* Задача: API, Чат, Базы данных.
* Spec: 1x AMD EPYC 7502P (32 ядра), 256 GB RAM, NVMe.
* Расчет: 3M соединений чата + БД + запас = 50 серверов.
* Стоимость: ~350$ в месяц

### Итоговая таблица

| Роль сервера | Конфигурация (CPU/RAM/Net) | Кол-во (шт) | Обоснование кол-ва |
| :--- | :--- | :--- | :--- |
| Ingest | 128 Cores / 256 GB / 100G | 1 780 | Обеспечение транскодинга для 100% стримов (89k) |
| Edge | 24 Cores / 128 GB / 100G | 252 | Исходящий трафик 8.4 Тбит/с |
| App & DB | 32 Cores / 256 GB / 20G | 50 | Бизнес-логика, чат, хранение данных |
| ВСЕГО | | 2 082 | |

### Итоговая оценка стоимости
| Категория серверов | Конфигурация | Количество | Цена аренды за шт. (мес) | Общая стоимость (мес) |
| :--- | :--- | :--- | :--- | :--- |
| Ingest  | 2x AMD EPYC 7763 (128 cores), 256 GB RAM | 1780 | 1 200 $ | 2 136 000 $ |
| Edge | 1x AMD EPYC 7402P (24 cores), 128 GB RAM, 50G Net | 252 | 1 650 $ | 415 800 $ |
| App & DB | 1x AMD EPYC 7502P (32 cores), 256 GB RAM | 50 | 350 $ | 17 500 $ |
| Итого | | 2082 | | 2 569 300 $ |
