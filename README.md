...
score = 2.5*(подписан) + 0.35*log1p(зрителей сейчас) + 0.25*(дельта зрителей за последние 5 минут) + 1.0*sim_cfollow + 0.10*(буст, если канал - партнер твича) + 0.20*(был повтор) + 0.05*(совпадение языка аккаунта и стрима)
